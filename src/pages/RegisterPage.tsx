...
import { UserPlus, Mail, Lock, User as UserIcon, Loader2, AlertCircle, CheckCircle2, LogIn } from "lucide-react";
...
export const RegisterPage: React.FC<RegisterPageProps> = ({ setActiveTab }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !email || !password) {
      setError("All fields (full name, email, and password) are required.");
      return;
    }
    ...
    <div className="max-w-md mx-auto my-8">
      <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl shadow-xs overflow-hidden">
        ...
        <div className="p-6 border-b border-stone-200 dark:border-stone-800 text-center space-y-1">
          <div className="w-12 h-12 bg-emerald-100 dark:bg-emerald-950/60 rounded-xl flex items-center justify-center text-emerald-600 dark:text-emerald-400 mx-auto mb-2">
          ...
          <p className="text-xs text-stone-500 dark:text-stone-400">
            Register to initialize your user account, profile, and secure resume storage.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-stone-400 text-white font-medium text-sm rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-2"
          >
            {isSubmitting ? (
              <>
